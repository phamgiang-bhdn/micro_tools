...
| `campaign` | Lọc theo merchant owner (vd `lazada`, `shopeenew`). |
...
**Tận dụng filter server-side** (sau STORY-03): `AccesstradeClient.fetchProducts(opts)` truyền `campaign`, `domain`, `price_from/to`, `discount_rate_from/to`, `status_discount`, `update_from/to` xuống `/v1/datafeeds`. Per-campaign loop trong `CrawlerService` đọc filter từ `Campaign.filterRules` (zod schema [filter-rules.dto.ts](../../apps/api/src/modules/crawler/dto/filter-rules.dto.ts)). `domains` whitelist vẫn filter client-side (AT chỉ hỗ trợ 1 domain/request).
...
**Gotcha quan trọng**:
- Endpoint trả **mọi campaign đã duyệt** của publisher. Per-campaign mode (STORY-03) chỉ pull những campaign có `Campaign.categoryId` + `atCampaignId` set sẵn, áp `filterRules` per row.
...
- Trước STORY-03 code upsert `Campaign` bằng `slugify(offer.campaign)` làm `externalId` (legacy). Sau STORY-02/03 path chính dùng `atCampaignId` thật từ `/v1/campaigns`; `resolveCampaignId` (legacy) chỉ còn fallback cho web-scrape.
...
**Triển khai trong repo** (STORY-02): `AccesstradeClient.fetchCampaigns({ approval, page, limit })` + `CampaignSyncService.syncFromAccesstrade()` (upsert theo `Campaign.atCampaignId`, không đụng admin-managed fields: `categoryId`, `filterRules`, `notes`). Trigger thủ công qua `POST /api/v1/admin/campaigns/sync-from-at` (role `admin`) hoặc nút "Sync from Accesstrade" trên `/admin/campaigns`.
...
| `campaigns.id` (từ `/v1/campaigns`) | `Campaign.atCampaignId` (khoá ổn định, set bởi `CampaignSyncService` STORY-02) |
...
| `order-list.order_id` | `ConversionWebhook.atOrderId` (đối soát theo đơn, set bởi `ReconciliationService` STORY-05) |
...
| `top_products.product_id` | `TopProductSnapshot.atProductId` (snapshot per day, STORY-07) |
...
2. **Datafeed trả mọi campaign approved** — pull không filter sẽ rất nhiều product. Sau sprint at-source-of-truth, crawler-cycle dùng **per-campaign loop**: lặp `Campaign` (status=APPROVED + có `categoryId` + `atCampaignId`) và truyền `campaign=<atCampaignId>` xuống `/v1/datafeeds` (server-side filter). `inferCategorySlug` (client-side) chỉ còn dùng cho path web-scrape paste URL — không đụng crawler-cycle.
3. **`campaign` field ở `/v1/datafeeds` là string (tên), không phải ID**. Đừng confuse với `data.id` ở `/v1/campaigns` (là số dài). Khi onboard endpoint campaigns mới, cần migration để link Campaign cũ (slug-based) với Campaign mới (id-based).
...
10. **`discount` ambiguous** *(đã fix ở STORY-03)*:
    ...
    - `AccesstradeClient.toNormalized` (sau STORY-03) đọc `discount_rate` cho `discountPercent`; `discount` được dùng làm fallback giá sau giảm (`sale = sale_price ?? discount ?? price`), không còn xem như %.
