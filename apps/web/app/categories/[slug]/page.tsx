...
import { fetchNicheBySlug, fetchNiches } from "../../../lib/api";
...
export async function generateMetadata({ params }: NichePageProps): Promise<Metadata> {
  ...
  const title = `${niche.name} — ${count} ưu đãi`;
  const description = `So sánh ${count} sản phẩm thuộc ${niche.name}. Giá cập nhật, chọn deal tốt nhất.`;
  return {
    title,
    description,
...
export default async function NicheDetailPage({ params }: NichePageProps): Promise<React.ReactElement> {
  const { slug } = await params;
  const [niche, nichesList] = await Promise.all([fetchNicheBySlug(slug), fetchNiches()]);
  if (!niche) notFound();

  const products = niche.products
    .map((p, idx) => ({ ...normalizeProduct(p), slug: niche.products[idx].slug ?? undefined, nicheSlug: niche.slug }))
  ...
  const otherNiches = (nichesList.niches ?? []).filter((c) => c.slug !== niche.slug).slice(0, 6);
        ...
        <PageContainer className="relative py-8 sm:py-10">
          ...
          <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="max-w-xl space-y-2">
              ...
              <h1 className="text-3xl font-bold tracking-tight text-ink sm:text-4xl">{niche.name}</h1>
