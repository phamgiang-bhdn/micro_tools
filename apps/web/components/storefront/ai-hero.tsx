...
export function AiHero({ tools, featuredDeal, productCount, hotDealCount }: Props): React.ReactElement | null {
  ...
  return (
    <section className="relative overflow-hidden border-b border-border bg-canvas">
      <div aria-hidden className="absolute inset-0 bg-ai-mesh" />
      <div className="relative mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 sm:py-14 lg:grid-cols-[1.25fr_1fr] lg:items-center">
        <div className="space-y-5">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-primary-200 bg-surface/80 px-3 py-1 text-micro font-semibold uppercase tracking-wider text-primary-700 backdrop-blur">
          ...
          <h1 className="text-3xl font-bold leading-tight tracking-tight text-ink sm:text-[2.75rem] sm:leading-[1.1]">
            Không biết chọn gì?{" "}
            <span className="text-gradient-ai">Để AI chọn giúp bạn</span> trong 60 giây.
          </h1>
          ...
          <Link
            ...
            className="ring-focus group flex items-center gap-3 rounded-2xl border border-border-strong bg-surface px-4 py-3.5 shadow-card transition hover:border-primary-400 hover:shadow-ai-glow"
          >
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-indigo-500 to-violet-600 px-3.5 py-1.5 text-sm font-semibold text-white shadow-ai-glow-sm transition group-hover:brightness-105">
          ...
          >
            <div className="relative aspect-square w-32 shrink-0 overflow-hidden rounded-xl bg-canvas">
              ...
              {typeof featuredDeal.discountPercent === "number" && featuredDeal.discountPercent > 0 ? (
                <span className="absolute left-2 top-2 rounded-md bg-cta-500 px-1.5 py-0.5 text-micro font-bold text-ink shadow">
