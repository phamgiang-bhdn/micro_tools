    ...
    <aside className="relative my-4 overflow-hidden rounded-2xl border border-brand-200 bg-gradient-to-br from-brand-50/60 to-white p-4 shadow-sm sm:p-5">
      ...
      <div className="flex gap-4">
        ...
        <div className="flex flex-1 flex-col">
          <p className="line-clamp-2 text-[14px] font-semibold leading-snug text-ink">{product.name}</p>

          {angle ? (
            <p className="mt-1 line-clamp-2 text-[12.5px] leading-snug text-ink-soft">{angle}</p>
          ) : null}

          {typeof pv.rating === "number" && pv.rating > 0 ? (
            <div className="mt-1.5 flex items-center gap-1 text-[11.5px] text-amber-700">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            {typeof pv.originalPrice === "number" && pv.originalPrice > (pv.price ?? 0) ? (
              <span className="text-[12px] text-ink-mute line-through">{formatMoney(pv.originalPrice)}</span>
            ) : null}
