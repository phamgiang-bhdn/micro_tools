    ...
    <section className="space-y-3">
      ...
      <div className="space-y-2">
        {items.map((item, i) => (
          <details
            key={i}
            className="group overflow-hidden rounded-2xl border border-line bg-card shadow-card transition open:border-brand-200 open:shadow-pop"
          >
            <summary className="flex cursor-pointer items-start justify-between gap-4 px-5 py-4 text-base font-semibold text-ink hover:text-brand-700 sm:px-6 sm:py-5">
              <span>{item.q}</span>
