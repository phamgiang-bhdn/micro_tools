...
export function VerdictBlock({ summary, bestFor, notFor }: Props): React.ReactElement {
  return (
    <section className="overflow-hidden rounded-3xl border border-ink/10 bg-gradient-to-br from-ink to-ink/90 p-7 text-white shadow-xl sm:p-9">
      <p className="text-micro font-semibold uppercase tracking-wider text-white/70">Kết luận</p>
        ...
        <div className="mt-7 grid gap-4 sm:grid-cols-2">
          {bestFor?.length ? (
            <div className="rounded-2xl bg-white/10 p-5 backdrop-blur-sm">
              <p className="text-micro font-semibold uppercase tracking-wider text-emerald-300">Phù hợp với</p>
          ...
          {notFor?.length ? (
            <div className="rounded-2xl bg-white/10 p-5 backdrop-blur-sm">
              <p className="text-micro font-semibold uppercase tracking-wider text-rose-300">Không hợp với</p>
              <ul className="mt-2.5 space-y-1.5">
                {notFor.map((p, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-white/90">
